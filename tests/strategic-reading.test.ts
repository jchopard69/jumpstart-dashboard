...
test('strategy includes a real reference and its measured peer group',()=>{const [signal]=buildStrategicReading(posts);assert.equal(signal.postId,'2');assert.match(signal.observation,/70 commentaires/);assert.match(signal.observation,/médiane de 15/);assert.match(signal.action,/Projet 2/);});
