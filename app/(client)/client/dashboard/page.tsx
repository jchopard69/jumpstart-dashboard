...
export default async function ClientDashboardPage({
  ...
  return <MonthlyWorkspace key={queryString} period={label(data.range)} previousPeriod={label(data.prevRange)} from={toIsoDate(data.range.start)} to={toIsoDate(data.range.end)} channels={channels} posts={data.posts.map(normalizeReviewPost)} rows={data.metrics} previousRows={data.prevMetrics} score={score} bestTimes={analyzeAccountBestTimes(data.posts,selectedAccounts)}
