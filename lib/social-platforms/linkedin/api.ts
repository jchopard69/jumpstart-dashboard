...
async function fetchNetworkSize(headers: Record<string, string>, organizationId: string): Promise<number> {
  ...
  const baseUrls = [API_REST_URL, API_V2_URL];
  for (const baseUrl of baseUrls) {
    for (const edgeType of edgeTypes) {
      const url = `${baseUrl}/networkSizes/${encodeURIComponent(orgUrn)}?edgeType=${edgeType}`;
      try {
...
export const linkedinConnector: Connector = {
  ...
  async sync({ externalAccountId, accessToken }) {
    ...
    if (latestDate && totalFollowers > 0) {
      ...
      if (entry) {
        // sync.ts detects this as the absolute anchor and keeps cumulative logic stable.
