...
type Result = { checked: number; meta: number; views: number; previews: number; unavailable: number; issues: number; done: boolean };
  ...
  const run=async()=>{
    ...
    try {
      while(state.index<clients.length && !stop.current){
        ...
        for(const row of data.results){total.checked++;if(['instagram','facebook'].includes(row.platform)){total.meta++;if(row.views)total.views++;}if(row.preview==='disponible')total.previews++;if(row.preview==='indisponible')total.unavailable++;if(row.issue)total.issues++;}
